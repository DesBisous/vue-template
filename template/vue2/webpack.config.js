...
const cdnConfig = config => {
  const cdnConfigData = { externals: {}, cdn: {} };
  if (cdnConfigData.externals) {
    // CDN 引入，比如： { jQuery: '$' }，html 上需要配合引入 <script src='jquery'/>
    config.externals = cdnConfigData.externals;
  }
  if (!cdnConfigData.cdn) return;
    ...
    config.optimization = {
      splitChunks: {
        cacheGroups: {
          ...
          /* antdv webpack split chunk config */
          echarts: {
            name: 'chunk-echarts',
